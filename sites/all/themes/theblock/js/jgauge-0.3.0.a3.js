...
function jGauge()
{
	 ...
	 */
        function init()
        {
                ...
                jQuery('#' + jG.id).css({'width': jG.width, 'height': jG.height});
                
                // Wipe the slate clean in case gauge already initialized.
                jQuery('#' + jG.id).children().remove();
                
                // Add the background image.
                jQuery('#' + jG.id).css('background-image', 'url("' + jG.imagePath + '")');
                ...
                htmlString = '<p id="' + jG.id + '-label" class="label">' + 
                        '<strong>' + jG.label.prefix + 
                        '</strong></p>';
                        
                jQuery('#' + jG.id).append(htmlString);
                
                // Position the main label...
                labelCssLeft = Math.round(jG.centerX - jQuery('#' + jG.id + '-label').getHiddenDimensions().w / 2 + jG.label.xOffset) + 'px';
                labelCssTop = Math.round(jG.centerY - jQuery('#' + jG.id + '-label').getHiddenDimensions().h / 2 + jG.label.yOffset) + 'px';
                jQuery('#' + jG.id + '-label').css({'left': labelCssLeft, 'top': labelCssTop});
                
                // Apply label color...
                jQuery('#' + jG.id + ' .label').css('color', jG.label.color);
                
                
                // Label created, so reveal it.
                jQuery('#' + jG.id + '-label').fadeIn('slow');
                ...
                jQuery('#' + jG.id + '-needle').load(function() {
                        // Position needle once it's done loading...
                        needleCssLeft = Math.round(jG.centerX - (jQuery('#' + jG.id + '-needle').getHiddenDimensions().w / 2) + jG.needle.xOffset) + 'px';
                        neeldeCssTop = Math.round(jG.centerY - (jQuery('#' + jG.id + '-needle').getHiddenDimensions().h / 2) + jG.needle.yOffset) + 'px';
                        jQuery('#' + jG.id + '-needle').css({'left': needleCssLeft, 'top': neeldeCssTop});
                });
                
                // Needle created, so reveal it.
                jQuery('#' + jG.id + '-needle').fadeIn('slow');
	...
	{
		jQuery('#' + jG.id).css('z-index', 0);
		jQuery('#' + jG.id + '-canvas-ranges').css('z-index', 1);
		jQuery('#' + jG.id + '-canvas-ticks').css('z-index', 2);
		jQuery('#' + jG.id + ' .tick-label').css('z-index', 3);
		jQuery('#' + jG.id + '-label').css('z-index', 4);
		jQuery('#' + jG.id + '-needle').css('z-index', 5); // This one doesn't work (overwritten by jQueryRotate??).
	}
	...
	function updateRange()
	{
		...
		jQuery('#' + jG.id + '-canvas-ranges').remove();
		...
		canvas = jQuery('#' + jG.id + '-canvas-ranges');
	...
	function updateTicks()
	{
		...
		// Remove any existing ticks canvas.
		jQuery('#' + jG.id + '-canvas-ticks').remove();
		...
		if (jG.ticks.count !== 0 || jG.ticks.thickness !== 0)
		{
			...
			// Reference the canvas element we just created.
			canvas = jQuery('#' + jG.id + '-canvas-ticks');
		...
		jQuery('#' + jG.id + ' .tick-label').remove();
		...
		if (jG.tickCount !== 0)
		{
			...
			for (i = 0; i <= jG.ticks.count - 1; i ++)
			{
				...
				leftOffset = jG.centerX + jG.needle.xOffset - jQuery('#' + jG.id + ' .tick-label').getHiddenDimensions().w / 2;
				topOffset = jG.centerY + jG.needle.yOffset - jQuery('#' + jG.id + ' .tick-label').getHiddenDimensions().h / 2;
				...
				jQuery('#' + jG.id + ' p:last').css({'left': tickLabelCssLeft, 'top': tickLabelCssTop});
			}

                        // Apply label color...
                        jQuery('#' + jG.id + ' .tick-label').css('color', jG.ticks.labelColor);
                        
			// Tick labels are all created, so reveal them together.
			jQuery('#' + jG.id + ' .tick-label').fadeIn('slow');
		}
	}
	...
	function setValue(newValue)
	{
		...
		// Rotate the needle...
		//jQuery('#' + jG.id + '-needle').rotateAnimation(jG.segmentStart + valueDegrees);
		jQuery('#' + jG.id + '-needle').rotate({ duration:3000, angle:0,animateTo:jG.segmentStart + 180 + valueDegrees,easing: jQuery.easing.easeOutBounce })
		//jQuery('#' + jG.id + '-needle').rotateAnimation(jG.segmentStart + valueDegrees-5);
		//jQuery('#' + jG.id + '-needle').rotateAnimation(jG.segmentStart + valueDegrees+5);
		jQuery('#' + jG.id + '-needle').css('position', 'absolute'); // This *MUST* be set after image-rotation!
		
		
		
		// Reposition needle (sometimes the needle moves off axis - witnessed in Firefox 3.6)...
		needleCssLeft = Math.round(jG.centerX - jQuery('#' + jG.id + '-needle').width() / 2 + jG.needle.xOffset) + 'px';
		needleCssTop = Math.round(jG.centerY - jQuery('#' + jG.id + '-needle').height() / 2 + jG.needle.yOffset) + 'px';
		jQuery('#' + jG.id + '-needle').css({'left': needleCssLeft, 'top': needleCssTop});
		
		// Build the label string (and apply any SI / binary prefix)...
		htmlString = prefixNumber(newValue, true);
		
		// Update label value...
		jQuery('#' + jG.id + '-label').html(htmlString);
	}
	...
	function prefixNumber(value, formatting)
	{
		...
		if (formatting === true)
		{
			return '<strong>' + jG.label.prefix + '</strong>';
		}
		else
		{

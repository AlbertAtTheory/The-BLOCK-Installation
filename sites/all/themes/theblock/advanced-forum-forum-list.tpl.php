...
<?php foreach ($tables as $table_id => $table): ?>
  ...
  <?php if ($table_info->tid==889) { ?><img src="/<?php print drupal_get_path('theme', 'theblock'); ?>/images/forum-sema-top.png" alt="NMCA World Street Finals" border="0" style="margin-bottom: -3px;" /><?php } ?>
  <div class="forum-table-wrap forum-wrapper-<?php print $table_info->tid; ?>">
    ...
    <div id="forum-table-<?php print $table_info->tid; ?>">
      <table class="forum-table forum-table-forums sticky-header">
        ...
        <tbody id="forum-table-<?php print $table_info->tid; ?>-content">
          <?php
          	foreach ($table['items'] as $item_id => $item):
          		$total_topics_plus = (int)$item->total_topics;
          		$total_posts_plus = (int)$item->total_posts;
          ?>
            <?php if ($item->is_container): ?>
              ...
              <td class="forum-details" colspan="<?php print $colspan ?>">
                ...
                <a href="/taxonomy/term/<?php print $item_id; ?>/feed" class="rss utility">RSS Feed</a>
                <a href="https://twitter.com/share?text=Check out the <?php print $item->name; ?> forum on The BLOCK!&url=http://theblock.com<?php print $item->link; ?>&related=ChevroletPerf" target="_blank" class="share-twitter utility">Share on Twitter</a>
                <a href="https://www.facebook.com/sharer.php?u=http://theblock.com<?php print $item->link; ?>" target="_blank" class="share-fb utility">Share on Facebook</a>
                ...
                <?php if (!empty($item->subcontainers)): ?>
                  <div class="forum-subcontainers">
                    <?php print $item->subcontainers; ?>
                  </div>
                <?php endif; ?>

                <?php
                	if (!empty($item->subforums)):
                		foreach ($item->subforum_list as $subforum_id => $subforum):
	                  		$total_topics_plus += (int)$subforum->total_topics;
	                  		$total_posts_plus += (int)$subforum->total_posts;
	                  	endforeach;
                ?>
